    ...
    return (
        <>
            <motion.div
                ...
                className="flex flex-col overflow-hidden rounded-3xl border border-zinc-800 bg-[#09090b] shadow-xl"
            >
                {/* Header */}
                <div className="flex items-center justify-between border-b border-zinc-800 p-6">
                    ...
                    <button
                        ...
                        className="flex items-center gap-1 rounded-full bg-zinc-900 px-3 py-1 text-xs font-medium text-zinc-400 transition-colors hover:bg-zinc-800 hover:text-white"
                    >
                ...
                <div className="p-6">
                    {/* Rank Card */}
                    <div className="relative overflow-hidden rounded-2xl bg-zinc-900 p-6 border border-zinc-800">
                        <div className="relative z-10 flex items-center justify-between">
                            <div className="flex items-center gap-4">
                                <div className={`flex h-14 w-14 items-center justify-center rounded-full border border-zinc-700 bg-zinc-800 text-3xl shadow-inner`}>
                                    {rank.icon}
                                </div>
                                <div>
                        ...
                        {sortedUnlockedBadges.length > 0 ? (
                            <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-zinc-800">
                                {sortedUnlockedBadges.map((badge, i) => (
                                    ...
                                    >
                                        <div className="flex h-16 w-16 items-center justify-center rounded-2xl border border-zinc-800 bg-zinc-900 text-3xl shadow-sm transition-all hover:scale-105 hover:border-zinc-700 hover:shadow-md hover:bg-zinc-800">
                                            {badge.icon}
                                        </div>
                    ...
                    {nextToUnlock && (
                        <div className="mt-4">
                            ...
                            <div className="group relative overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-900 p-1 transition-all hover:border-zinc-700">
                                <div className="flex items-center gap-4 p-4">
                                    <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-zinc-950 border border-zinc-800 text-zinc-600 group-hover:text-zinc-400 transition-colors">
                                        <Lock className="h-5 w-5" />
                                    </div>
                                    <div className="flex-1">
                                        <div className="flex items-center justify-between">
                                            <div className="text-sm font-bold text-white">{nextToUnlock.name}</div>
                ...
                <AnimatePresence>
                    ...
                    >
                        <motion.div
                            ...
                            className="flex max-h-[85vh] w-full max-w-3xl flex-col overflow-hidden rounded-3xl border border-zinc-800 bg-[#09090b] shadow-2xl"
                        ...
                        >
                            <div className="flex items-center justify-between border-b border-zinc-800 p-6">
                                ...
                                <button onClick={() => setShowModal(false)} className="rounded-full bg-zinc-900 p-2 text-zinc-400 hover:bg-zinc-800 hover:text-white transition-colors">
                                    <X className="h-5 w-5" />
                                </button>
                            </div>
                                    ...
                                    {badges.map((badge, i) => (
                                        <motion.div
                                            ...
                                            className={`relative flex flex-col items-center overflow-hidden rounded-2xl border p-5 text-center transition-all ${badge.unlocked
                                                ? "border-zinc-700 bg-zinc-900 shadow-lg ring-1 ring-white/10"
                                                : "border-zinc-800 bg-zinc-950 opacity-50 grayscale"
                                                }`}
                                        >
                                            <div className="mb-3 text-4xl drop-shadow-md">{badge.icon}</div>
                                            <div className="mb-1 text-sm font-bold text-white">{badge.name}</div>
