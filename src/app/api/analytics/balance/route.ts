...
export async function GET(req: Request) {
  try {
    ...
    for (const r of rows) {
      if (!map.has(r.month))
        map.set(r.month, { month: r.month, income: 0, expense: 0 });
      const row = map.get(r.month)!;
      if (r.type === "income") row.income += r.total;
      else if (r.type === "expense") row.expense += r.total;
    }
    const byMonth = Array.from(map.values()).map((r) => ({
      month: r.month,
      net: r.income - r.expense,
    }));
    let cumulative = 0;
    const withCumulative = byMonth.map((r) => {
      cumulative += r.net;
      return { ...r, cumulative };
    });

    return NextResponse.json({ data: withCumulative });
  } catch (err: any) {
